...
{synopt: {bf:missingok}}Suppresses error when the pipe does not exists in any variable label{p_end}
{synoptline}
...
The {inp:lbl_list_pipes} command can be used to list which pipes
exists in the dataset,
and then {inp:lbl_replace_pipe} can be used to replace the pipes.
{p_end}

{pstd}{inp:lbl_replace_pipe} can only replace one pipe at the time.
...
{pstd}{bf:pipe}({it:string}) is the option that indicates which
...
{pstd}{bf:{ul:rep}lacement}({it:string}) is the value that
...
{pstd}{bf:{ul:trun}cate}({it:string}) is an option that let the user
decide what should happen if a label is too long
after the pipe has been replaced with the new value.
The options are {inp:error} (the commands throws an error and exits),
{inp:warning} (the command outputs a warning and continues) and
{inp:prompt} (the command askes the user to interactively confirm each case).
{p_end}
...
{pstd}{bf:missingok} suppresses the error thrown if a pipe the user
